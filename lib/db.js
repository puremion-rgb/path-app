...
function createSchema(db) {
  db.exec(`
    ...
    -- "AI에게 질문하기" 화면(번역/일반 여행 지식 질문) 전용 대화 기록입니다.
    -- 일정 수정 대화(ChatMessage)와 완전히 분리해서, 두 화면의 대화가 서로
    -- 섞여 보이지 않게 합니다.
    CREATE TABLE IF NOT EXISTS AskMessage (
      id TEXT PRIMARY KEY,
      userId TEXT NOT NULL REFERENCES User(id) ON DELETE CASCADE,
      tripId TEXT REFERENCES Trip(id) ON DELETE SET NULL,
      role TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_askmessage_trip ON AskMessage(tripId);
