...
// image: 메뉴판/표지판 사진을 첨부한 경우 "data:image/jpeg;base64,..." 형태의 data URL.
export function askAi({ tripId, message, image }) {
  return jsonFetch("/api/agent/ask", { method: "POST", body: JSON.stringify({ tripId, message, image }) }).then(
    (data) => data.answer
  );
}
